...
public class CsvMessageConverter<T> extends AbstractGenericHttpMessageConverter<List<T>> {
	...
	public boolean canRead(final Type type, final Class<?> clazz, final MediaType mediaType) {
		if(type instanceof ParameterizedType) {
			...
			return super.canRead(mediaType) && mediaType.equals(SUPPORTED_MEDIA_TYPE) && mediaType != null && Class.forName(parameterizedType.getRawType().getTypeName()).isAssignableFrom(List.class);
		}
		return false;
	}
	...
	public boolean canWrite(final Type type, final Class<?> clazz, final MediaType mediaType) {
		final Class listType = type instanceof Class ? type.getClass() : (Class) ((ParameterizedType) type).getRawType();
