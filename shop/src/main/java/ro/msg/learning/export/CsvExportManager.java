...
public class CsvExportManager {
	...
	public CsvExportManager(final @Qualifier("csvMapper") CsvMapper csvMapper) {
		this.csvMapper = csvMapper;
	}

	@SneakyThrows
	public <T> void toCsv(final Class<T> clazz, final List<T> pojosToSerialize, final OutputStream outputStream)
	...
	public <T> List<T> fromCsv(final Class<T> clazz, final InputStream inputStream) {
		final CsvSchema schema = csvMapper.schemaFor(clazz).withHeader();
		final MappingIterator<T> it = csvMapper.readerFor(clazz).with(schema).readValues(inputStream);
